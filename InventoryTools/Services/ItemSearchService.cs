...
public class ItemSearchService(MediatorService mediatorService, IClientState clientState, InventoryToolsConfiguration configuration, IInventoryMonitor inventoryMonitor, ICharacterMonitor characterMonitor, InventoryScopeCalculator calculator, IChatGui chatGui, IDataManager dataManager) : IHostedService, IMediatorSubscriber, IDisposable
{
    ...
    private void ItemSearchRequested(ItemSearchRequestedMessage searchRequest)
    {
        ...
        var item = dataManager.GetExcelSheet<ItemEx>()!.GetRow(searchRequest.ItemId);
        if (item == null)
        {
            return;
        }

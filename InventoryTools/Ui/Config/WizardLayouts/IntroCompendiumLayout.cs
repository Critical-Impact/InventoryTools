...
public class IntroCompendiumLayout : ContentLayout
{
    public override PageLayout Build()
    {
        return Page("intro/compendium", "Compendium",
            Paragraph(
                "The compendium is a browsable reference for the game's content, built from the same data the plugin uses everywhere else. If you want to know what something is, where it comes from or what drops it, this is where to look."),
            ...
            Section("Have a look",
                Paragraph(
                    "Each entry links through to everything related to it, so you can follow a chain from an item to the duty that drops it to the NPC that sells the rest."),
                OpenWindow<CompendiumTypesWindow>("Open the compendium"),
                Paragraph(
                    "It also lives under the Compendium menu in the items and crafts windows, or the /compendium command."))
        );
    }
}

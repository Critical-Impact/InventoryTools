        ...
        public override unsafe void Draw()
        {
            ...
            if (ImGui.BeginChild("###ivDebugView", new Vector2(-1, -1), true))
            {
                ...
                else if (ConfigurationManager.Config.SelectedDebugPage == 2)
                {
                    ...
                    {
                        PluginLog.Log($"item order module : {(ulong)ItemOrderModule.Instance()->SaddleBagPremium:X}", $"{(ulong)ItemOrderModule.Instance()->SaddleBagPremium:X}");
                        PluginLog.Log($"item order module : {(ulong)ItemOrderModule.Instance():X}", $"{(ulong)ItemOrderModule.Instance():X}");
                        PluginLog.Log($"slots per container : " + ItemOrderModule.Instance()->SaddleBagPremium->SlotPerContainer);
                    ...
                    {
                        PluginLog.Log($"item order module : {(ulong)ItemOrderModule.Instance()->Retainers:X}", $"{(ulong)ItemOrderModule.Instance()->Retainers:X}");
                        PluginLog.Log($"item order module : {(ulong)ItemOrderModule.Instance():X}", $"{(ulong)ItemOrderModule.Instance():X}");
                        ...
                        PluginLog.Log(ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag->SlotPerContainer.ToString());
                        PluginLog.Log($"item order module : {(ulong)ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag:X}", $"{(ulong)ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag:X}");
                        for (int i = 0; i < ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag->SlotPerContainer * 5; i++)
                        {
                            var slotIndex = ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag->Slots[i]->SlotIndex;
                            var containerIndex = ItemOrderModule.Instance()->Retainers->SortOrders->Retainer1Bag->Slots[i]->ContainerIndex;
                            PluginLog.Log(containerIndex.ToString() + ":" + slotIndex.ToString());
                        }
                    }
                    if (ImGui.Button("Memory sort check"))
                    {
                        ...
                        if (parsedItemOrder.NormalInventories.ContainsKey("PlayerInventory"))
                        {
                            ...
                            {
                                PluginLog.Log("Player Inventory: " + slot.containerIndex.ToString() + " : " + slot.slotIndex);
                            }
                        }
                    ...
                    if (ImGui.Button("Check free company tab"))
                    {
                        ...
                        var inventoryLarge = PluginService.GameUi.GetWindow(CriticalCommonLib.Services.Ui.WindowName.FreeCompanyChest.ToString());
                        if (inventoryLarge != null)
                        {
                ...
                else if (ConfigurationManager.Config.SelectedDebugPage == 5)
                {
                    ...
                    if (craftMonitorAgent != null)
                    {
                        ...
                        ImGui.Text("Item: " + (Service.ExcelCache.GetItemExSheet().GetRow(craftMonitorAgent.ResultItemId)?.NameString ?? "Unknown"));
                    ...
                    else if (simpleCraftMonitorAgent != null)
                    {
                        ...
                        ImGui.Text("Item: " + Service.ExcelCache.GetItemExSheet().GetRow(simpleCraftMonitorAgent.ResultItemId)?.NameString.ToString() ?? "Unknown");
                        ImGui.Text("Current Recipe: " + PluginService.CraftMonitor.CurrentRecipe?.RowId ?? "Unknown");
                        ImGui.Text("Current Craft Type: " + PluginService.CraftMonitor.Agent?.CraftType ?? "Unknown");
                    }
                    else
                            ...
                            if (container != null) {
                                ...
                                if (ImGui.TreeNode("Items##containerItems")) {
                                    ...
                                    for (var i = 0; i < container->Size; i++) {
                                        ...
                                        Utils.PrintOutObject(item, (ulong) &item, new List<string> {$"Items[{i}]"},false, $"[{i:00}] {actualItemName}" );
                                    }
                                    ImGui.TreePop();
                                }
                            } else {
                ...
                else if (ConfigurationManager.Config.SelectedDebugPage == 8)
                {
                    ...
                    if (UIState.Instance()->Cabinet.IsCabinetLoaded())
                    {
                        ...
                        foreach (var row in Service.ExcelCache.GetCabinetSheet().OrderBy(c => c.Category.Row).ThenBy(c => c.Order))
                        {
                            ...
                            var memoryInventoryItem = CriticalCommonLib.Models.InventoryItem.FromArmoireItem(isInArmoire ? itemId : 0, (short)index);
                ...
                else if (ConfigurationManager.Config.SelectedDebugPage == 10)
                {
                    ...
                    if (ImGui.TreeNode("Gearsets##gearsets"))
                    {
                        ...
                        {
                            if (ImGui.TreeNode("Gearset " + PluginService.InventoryScanner.GearSetNames[gearSet.Key] + "##gil" + gearSet.Key))
                            {

            ...
            SearchResult searchResult, int rowIndex, int columnIndex)
        {
            ...
            if (searchResult.Item.CanTryOn)
            {
                ...
                if (ImGui.SmallButton("Try On##TO" + rowIndex))
                {
                    ...
                    {
                        _tryOn.TryOnItem(searchResult.Item, searchResult.InventoryItem?.Stain ?? 0, searchResult.InventoryItem?.IsHQ ?? false);
                    }
                    else
                    {
                        Logger.LogError("Something went wrong while attempting to try on " + searchResult.Item.NameString);
                    }
                }
            }
